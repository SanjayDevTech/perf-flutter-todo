    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `todos` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `content` TEXT NOT NULL)');
...
class _$TodoDao extends TodoDao {
  _$TodoDao(
    ...
    this.changeListener,
        ...
        _todoEntityInsertionAdapter = InsertionAdapter(
            ...
            'todos',
            (TodoEntity item) =>
                <String, Object?>{'id': item.id, 'content': item.content},
            changeListener),
  ...
  Stream<List<TodoEntity>> getAll() {
    return _queryAdapter.queryListStream('SELECT * FROM todos',
        mapper: (Map<String, Object?> row) =>
            TodoEntity(row['id'] as int?, row['content'] as String),
        queryableName: 'todos',
        isView: false);
  }
  ...
  Future<void> insert(TodoEntity todo) async {
    await _todoEntityInsertionAdapter.insert(todo, OnConflictStrategy.abort);
  }

  @override
  Future<void> update(TodoEntity todo) async {
    await _todoEntityUpdateAdapter.update(todo, OnConflictStrategy.abort);
  }
}
